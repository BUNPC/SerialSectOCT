...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9870 to make a header
// TR acquisition.
//
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    // Create a record header file if required
    FILE *fpHeader = NULL;

    if (enableHeaders)
    {
        fpHeader = fopen("header.txt", "w");
        if (fpHeader == NULL)
        {
            printf("Error: Unable to create record header file -- %u\n", GetLastError());
            return FALSE;
        }

        // Write column headers to record header file
        if (fprintf(fpHeader, "Record \tCH \tHeader0 \tHeader1 \tHeader2 \tHeader3\n") < 0)
        {
            printf("Error: Write column headers to record header file failed -- %u\n",
                   GetLastError());
            fclose(fpHeader);
            return FALSE;
        }
    ...
    for (bufferIndex = 0; (bufferIndex < BUFFER_COUNT) && success; bufferIndex++)
    {
        ...
        BufferArray[bufferIndex] =
            (U8 *)AlazarAllocBufferU8Ex(boardHandle, bytesPerBuffer);
                ...
                for (U32 record = 0; (record < recordsPerBuffer) && (success == TRUE); record++)
                {
                         ...
                         channel++)
                    {
                        if (enableHeaders)
                        {
                            ...
                            // Write write record header to file
                            if (fprintf(fpHeader,
                                        "%u \t%c \t0x%08X \t0x%08X \t0x%08X \t0x%08X\n", record,
                                        channelLetter, pHeader->hdr0, pHeader->hdr1,
                                        pHeader->hdr2, pHeader->hdr3) < 0)
                            {
                                printf(
                                    "Error: Write record %u channel %c header failed -- %u\n",
                                    record, channelLetter, GetLastError());
                                success = FALSE;
                            }
                        ...
                        if (success && saveData)
                        {
                            ...
                            size_t bytesWritten =
                                fwrite(pRecord, sizeof(BYTE), bytesPerRecord, fpData);
