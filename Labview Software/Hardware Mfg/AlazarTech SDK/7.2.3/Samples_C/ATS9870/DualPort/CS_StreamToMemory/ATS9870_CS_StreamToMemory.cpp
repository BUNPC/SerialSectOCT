...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9870 to make a streamtomemory
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    U8 *data = (U8 *)AlazarAllocBufferU8Ex(boardHandle, bytesPerAcquisition);
    ...
    while (totalBuffersPosted < POSTED_BUFFER_COUNT && totalBuffersPosted < transfersPerAcquisition) {
        U8* pBuffer = data + bytesPerTransfer * totalBuffersPosted;
    ...
    if (success)
    {
        ...
        while (transfersCompleted < transfersPerAcquisition)
        {
            ...
            U32 timeout_ms = 5000;
            U8 *pBuffer = data + transfersCompleted * bytesPerTransfer;
            ...
            if (totalBuffersPosted < transfersPerAcquisition) {
                pBuffer = data + totalBuffersPosted * bytesPerTransfer;
                retCode = AlazarPostAsyncBuffer(boardHandle, pBuffer, bytesPerTransfer);
                if (retCode != ApiSuccess)
                {
    ...
    if (data != NULL)
    {
        AlazarFreeBufferU8X(data, boardHandle);
    }
