...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9373 to make a des
// CS acquisition.
//
...
BOOL ConfigureBoard(HANDLE boardHandle)
{
    ...
    // TODO: Specify the channel mask.
    //
    // NOTE: Do not enable more than one channel if the acquisition rate is above 2000 MS/s
    channelMask = CHANNEL_A;

    if (samplesPerSec > 2e9)
    {
        retCode = AlazarSetParameterUL(boardHandle, channelMask, SET_ADC_MODE, ADC_MODE_DES);
        if (retCode != ApiSuccess)
        {
            printf("Error: AlazarSetParameter (SET_ADC_MODE) failed -- %s\n",
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    if (success)
    {
        U32 admaFlags = ADMA_EXTERNAL_STARTCAPTURE | ADMA_CONTINUOUS_MODE | ADMA_FIFO_ONLY_STREAMING;
    ...
    // Set the board in single channel mode if the acquisition rate is more than 2 GS/s
    if (samplesPerSec > 2e9)
    {
        retCode = AlazarSetParameter(boardHandle, 0, SET_SINGLE_CHANNEL_MODE, channelMask);
        if (retCode != ApiSuccess)
        {
            printf("Error: AlazarSetParameter(SET_SINGLE_CHANNEL_MODE) failed -- %s\n",
                   AlazarErrorToText(retCode));
            success = FALSE;
        }
    }
