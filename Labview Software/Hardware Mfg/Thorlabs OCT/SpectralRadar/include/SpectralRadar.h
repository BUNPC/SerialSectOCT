...
#define FALSE 0
	...
	SPECTRALRADAR_API BOOL isDeviceAvailable();
	SPECTRALRADAR_API void expectedDevice(char* Buffer, int Size, int* Rev);
    ...
    ///
	/// \fn ScanPatternHandle createPointScanPattern(ProbeHandle Probe, int Size, double PosX, double PosY);
	/// \ingroup ScanPattern
	/// \brief Creates a scan pattern that consists of a single point (PosX, PosY). The galvo doesn't move from there. Use this pattern for point scans and/or non-scanning probes.
    ///
	...
	/// The complete volume will be returned in one raw data (#RawDataHandle).
    ///
	...
	/// The volume will be returned slice-by-slice by calling #getRawData().
    ///
	...
	///
	/// \fn ScanPatternHandle createFragmentedScanPattern(ProbeHandle Probe, int ChunkSize, int NumberOfChunks);
	/// \ingroup ScanPattern
	...
	typedef enum {
		/// Specifies an asynchronous infinite/continuous measurement. The internal memory management is constructed with a loop of buffers. Note that you may lose data if the acquisition is faster than the collection with #getRawData(). 
		Acquisition_AsyncContinuous,
		/// Specifies an asynchronous finite measurement. The reserved memory for the data is as large as required to make sure that no data will be lost.  
		Acquisition_AsyncFinite,
		/// Specfies a synchronous measurement. Each measurement of e.g. a single B-scan will be started with #getRawData(). Since the acquisition is synchronized with the software it is not a continious measurement over the time of e.g. several B-scans.
		Acquisition_Sync
	} AcquisitionType;
    ...
    ///
	/// \fn double getSettingsEntryDouble(SettingsHandle, const char*, double);
	/// \ingroup Settings
	...
	typedef enum {
		...
		/// maximal external trigger frequency for the chosen preset
	...
	typedef enum {
		/// The scan pattern will be acquired slice by slice which means that the function GetRawData() needs to be called more than once to get the data for the whole scan pattern
		ScanPattern_AcqOrderFrameByFrame, 
		/// The scan patten will be acquired in one piece
		ScanPattern_AcqOrderAll 
	} ScanPattern_AcquisitionOrder;
	...
	/// \fn void setTriggerMode(OCTDeviceHandle Dev, Device_TriggerType Trigger, double Timeout);
	/// \ingroup External Trigger
	/// \brief Sets the trigger mode for the OCT device used for acquisition. Additional hardware may be needed. The timeout must be given in seconds.
	///
	/// \fn Device_TriggerType getTriggerMode(OCTDeviceHandle Dev);
	/// \ingroup External Trigger
	/// \brief Returns the trigger mode used for acquisition.
	///
	/// \fn bool isTriggerModeAvailable(OCTDeviceHandle Dev, Device_TriggerType TriggerMode);
	/// \ingroup External Trigger
	/// \brief Returns whether the specified trigger mode is possible or not for the used device. 
	///
	/// \fn int getTriggerPropertyInt(OCTDeviceHandle Dev, Device_TriggerPropertyInt TriggerProperty);
	/// \ingroup External Trigger
	/// \brief Returns the specified trigger property. 

	/// \fn void setTriggerTimeoutSec(OCTDeviceHandle Dev, double Timeout);
	/// \ingroup External Trigger
	/// \brief Sets the timeout of the camera (useful in external trigger mode). 

	/// \fn double getTriggerTimeoutSec(OCTDeviceHandle Dev);
	/// \ingroup External Trigger
	/// \brief Returns the timeout of the camera (not used in trigger mode Trigger_FreeRunning). 

	SPECTRALRADAR_API void setTriggerMode(OCTDeviceHandle Dev, Device_TriggerType TriggerMode);
	SPECTRALRADAR_API Device_TriggerType getTriggerMode(OCTDeviceHandle Dev);
	SPECTRALRADAR_API bool isTriggerModeAvailable(OCTDeviceHandle Dev, Device_TriggerType TriggerMode);
	SPECTRALRADAR_API void setTriggerTimeoutSec(OCTDeviceHandle Dev, int Timeout);
	SPECTRALRADAR_API int getTriggerTimeoutSec(OCTDeviceHandle Dev);
	...
	SPECTRALRADAR_API double expectedAcquisitionTimeSec(ScanPatternHandle ScanPattern, OCTDeviceHandle Dev);
...
// USB controller
	...
	SPECTRALRADAR_API BOOL refstageAvailable(OCTDeviceHandle Dev);
	SPECTRALRADAR_API RefstageStatus refstageGetStatus(OCTDeviceHandle Dev);
	SPECTRALRADAR_API double refstageGetLength_mm(OCTDeviceHandle Dev, ProbeHandle Probe);
	SPECTRALRADAR_API double refstageGetPosition_mm(OCTDeviceHandle Dev, ProbeHandle Probe);
	SPECTRALRADAR_API void refstageHome(OCTDeviceHandle Dev, BOOL wait);
	SPECTRALRADAR_API void refstageMoveLonger(OCTDeviceHandle Dev);
	SPECTRALRADAR_API void refstageMoveShorter(OCTDeviceHandle Dev);
	SPECTRALRADAR_API void refstageMoveAbsolute(OCTDeviceHandle Dev, ProbeHandle Probe, double pos_mm);
	SPECTRALRADAR_API void refstageStop(OCTDeviceHandle Dev);
	SPECTRALRADAR_API void refstageSetSpeed(OCTDeviceHandle Dev, RefstageSpeed speed);
	SPECTRALRADAR_API void refstageSetStatusCallback(OCTDeviceHandle Dev, cbRefstageStatusChanged Callback);
	SPECTRALRADAR_API void refstageSetPosChangeCallback(OCTDeviceHandle Dev, cbRefstagePositionChanged Callback);
