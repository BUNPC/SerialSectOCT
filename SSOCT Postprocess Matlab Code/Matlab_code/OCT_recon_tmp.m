...
datapath  = '/projectnb2/npbssmic/ns/BA4445_4/';
P2path = '/projectnb2/npbssmic/ns/BA4445_4_2P/';   % 2P file path
nslice=120; % define total number of slice
...
cd(datapath);
filename0=dir(strcat('120-*AB.dat')); % count #tiles per slice
...
if ~isfile(strcat(datapath,'surface.mat'))
    Hui_sum_all(datapath,ntile, 5,300); % parameters: data path, ntile, slice#, Z pixels. slice# should be the one that was cut even.
end
cd(datapath)
...
%              ori = Grid_correction(ori, grid_matrix, 1050, 51, 1050, 51, size(ori,1));  
         ...
         start_pixel=50; % start depth needs to be configured for each sample !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         sur=round(Vq);
         ori2D=Gen_ori_2D(ori,sur,30);
         
         % saving corrected tiles to folder. Optional
         if(mean2(aip)>0.07 || std2(aip)>0.01)
...
%    Ori_stitch('ori2D', P2path,datapath,disp,mosaic,pxlsize,islice,pattern,sys); 
 ...
 %   BaSiC_shading_and_ref_stitch(islice,P2path,datapath, numX*numY, start_pixel+35, 70); % volume recon start depth, thickness(pixels)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
