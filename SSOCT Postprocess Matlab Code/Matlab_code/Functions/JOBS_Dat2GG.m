%% Generate job file for launchpad calculating Dat2GG
function JOBS_Dat2GG(filename0,datapath0,Start_file,N_file,jobsavepath)

jobfilename='Dat2GG.sh';
if nargin<5
...
prompt2={'RR high pass filter?','Save all RR?','Number of speckle angiography?','Repeat Bscan for ACF?'...
    ...
    'intDk(-0.19 for BU Anes, -0.235 for BU Awake)','Aline Rate (kHz)'};
inputNum2=inputdlg(prompt2,'', 1,{'0','0','3',num2str(RptBscan),...
    '1','0','0','1', num2str(OVERLAP_Ascan),'-0.19','76'});
...
trunc_z=str2num(inputNum2{8});   % 
OVERLAP_Ascan=str2num(inputNum2{9});  % 
intDk=str2num(inputNum2{10});  % 
fDAQ=str2num(inputNum2{11});  % 
...
n=1.35;
dZ=2*log(2)*lam0_s.^2/(pi*lam_bw*n)*(nk/Num_pixel);
LengthZ=floor(100/(nk/Num_pixel)); % default length of Z_seg
%%  Check surface %%%%%%
...
%     prompt2={'Surface','Start Z_seg',['Length Z_seg (axial resolution: ',num2str(dZ),')']};
    ...
    z_seg0=str2num(inputZseg{2});  % number of segments
    LengthZ=str2num(inputZseg{3})-str2num(inputZseg{2})+1;
    DC=0;
end
%%%%
Dim=[nk, nxRpt, nx, nyRpt, ny0];
XYTTauDim=[Num_Aline,Num_Bscan,RptA_start,RptA_Interval, RptA_n, ACF_Start,ACF_nt,ACF_nTau];
Proc_option=[MW_AVG,Lowpassfilt,Image_shift,OVERLAP_Ascan,GG_Neib_AVG,trunc_z,z_seg0,LengthZ,NeibNorm,DC,RRHPF,SaveRRall, Nspk_angio, RptBscan,intDk, fDAQ];  
datapath=['',datapath0,''];
%% path information 
...
inputSCCinfo=inputdlg(prompt3,'SCC request parameter', 1,{num2str(N_Ysegs),'2','8','6',jobname,'N'});
...
JobName=['Dat2GG-',inputSCCinfo{5}];              % Job name
...
for ifile=1:N_file
    ...
    job_cmd=['#! /bin/bash -l','\n'...
        ...
        'i_Yseg=$SGE_TASK_ID; ',...
        'SCC_DLSOCT_A_Dat2GG(datapath, filename0,Proc_option, Dim, XYTTauDim, Num_pixel,N_Ysegs,i_Yseg); ',...
        'diary off; exit"']];
    fprintf(fid,job_cmd);
end
fclose(fid);
...
%% save Dat2GG process information
%%%%
for ifile=1:N_file
...
fclose(fid);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp(['Job Dat2GG.txt saved,', datestr(now,'DD:HH:MM')]);
disp(savepath);
