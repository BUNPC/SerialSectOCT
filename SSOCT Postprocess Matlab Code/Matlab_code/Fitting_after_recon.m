...
folder = '/projectnb2/npbssmic/ns/Ann_Mckee_samples_10T/NC_6974_2/';      % OCT file path                          ADJUST FOR EACH SAMPLE!!! 
P2path = '/projectnb2/npbssmic/ns/Ann_Mckee_samples_10T/NC_6974_2P/';   % 2P file path                           ADJUST FOR EACH SAMPLE!!!
datapath=strcat(folder,'dist_corrected/'); 
nslice=28; % define total number of slices                                                                       ADJUST FOR EACH SAMPLE!!!
...
%       Optical_fitting_finalized(co, cross, s_seg, z_seg, datapath,aip_threshold, mus_depth, bfg_depth, ds_factor, zf, zf_tilt_x, zf_tilt_y)
        Optical_fitting_finalized(co, cross, islice, iFile, folder,      0.037,       130,       100,     ds_factor, 105,    10,       5);
        % aip_threshold should use the same value in OCT_recon.m
        
        % Find zf by evaluating the agarose tiles using view3D(co)
        % zf is the focus depth at center of FOV in pixels, including water space

        % zf_tilt_x and zf_tilt_y is the zf different along x and y
        % dimension. there are two shapes of zf tilt in
        % optical_fitting_finalied: V shape and linear shape. Choose your
        % appropriate shape after evaluating agarose tile using view3D(co).
        % If zf_tilt_x = 0 or zf_tilt_y = 0, you are using flat zf.
    end
