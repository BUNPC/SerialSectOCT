...
P2path = '/projectnb2/npbssmic/ns/CTE_6894_2P/';  % 2P data path.
% leave unchanged if no 2P data is avalable                                                          ADJUST FOR EACH SAMPLE!!!
nslice=42;   % total number of imaging slices.                                                       ADJUST FOR EACH SAMPLE!!!
...
highres=1;% 1 means 2PM is imaged at 1um steps size, 0 means 2PM is imaged at 2um step size          ADJUST FOR EACH SAMPLE!!!
...
xx=543;    % xx is the X displacement of two adjacent tile align in the X direction
xy=0;     % xy is the Y displacement of two adjacent tile align in the X direction, default to 0
yy=543;    % yy is the Y displacement of two adjacent tile align in the Y direction
...
ntile=numX*numY;
start_pixel=55; % start depth for calculating MIP, retardance, and volume reconstruction. 
...
id =str2num(id);                                                                                % change to 1 for debugging
...
if ~isfile(strcat(OCTpath,'surface.mat'))
    if id==1
        Hui_sum_all(OCTpath,ntile, round(nslice/4)*2+1,Zsize,700); % parameters: data path, ntile, slice#, Z pixels. slice# should be the one that was cut even.
    else
        pause(7200)
    end
end
...
%                  cross = cross(:,1:end,1:end);
%                  co = co(:,26:end-25,26:end-25);
