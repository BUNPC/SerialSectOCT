...
datapath  = '/projectnb2/npbssmic/ns/Ann_Mckee_samples_20T/NC_7597/';
P2path = '/projectnb2/npbssmic/ns/Ann_Mckee_samples_20T/NC_7597_2P/';   % 2P file path
nslice=24; % define total number of slice
...
%              ori = Grid_correction(ori, grid_matrix, 1050, 51, 1050, 51, size(ori,1));  
         ...
         start_pixel=107; % start depth needs to be configured for each sample !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
%     Gen_OCT_coord(datapath,disp,mosaic,pxlsize,islice,pattern, 5, 9, 15)
    AIP_stitch_Ann(P2path,datapath,disp,mosaic,pxlsize,islice,pattern,sys,0);                     % stitch AIP
%     Mus_stitch('mus',P2path,datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);           % stitch mus
%     Mub_stitch('mub', P2path,datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);           % stitch mub
% %     rewrite_bfg_tiles(datapath,islice,numX*numY);
%     Bfg_stitch('bfg', P2path,datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);
%     R2_stitch('R2', P2path, datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);
%     ZF_stitch('ZF', P2path, datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);
%     ZR_stitch('ZR', P2path, datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);
%     BKG_stitch('BKG', P2path, datapath,disp,mosaic,pxlsize./10,islice,pattern,sys);
    MIP_stitch_Ann('mip', P2path,datapath,disp,mosaic,pxlsize,islice,pattern,sys,0);
%     Surf_stitch('sur',P2path,datapath,disp,mosaic,pxlsize/10,islice,pattern,sys);              % stitch surface
    Ret_stitch_Ann('ret_aip', P2path,datapath,disp,mosaic,pxlsize,islice,pattern,sys,0);              % stitch retardance AIP
    Ori_stitch_Ann('ori2D', P2path,datapath,disp,mosaic,pxlsize,islice,pattern,sys,0); 
    % Gen_ori_RGB(datapath,islice, 0.1);
    BaSiC_shading_and_ref_stitch_Ann(islice,P2path,datapath, numX*numY, start_pixel, 70, 0); % volume recon start depth, thickness(pixels)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
