function Optical_fitting_Hui_0415(ref, s_seg, z_seg, datapath)
...
%         mask2(:,:,i)=mask;
%     end
    vol=ref;%mask2.*ref./1000;

    %% Curve fitting for the whole image
    fit_depth = round(120); 
    res = 2;  %A-line averaging factor
    est_pix = zeros(round(size(vol,1)/res),round(size(vol,2)/res),4);
%     tmp=round(imresize(sur,0.2));
    for i = 1:round(size(vol,1)/res)
        for j = 1:round(size(vol,2)/res)
...
%             xloc=sur(ceil(i/10*res),ceil(j/10*res))+2;
            if m > 0.05
                ...
                fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./p(4)).^2))) +p(5); %75; 3-parameter fitting using empirical rayleigh range
    %             fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-est(3))./(est(4))).^2))); % 2-parameter fitting using tile-dependent zf and rayleigh range from above fitting
                ...
                lb = [0.1 0.0001 150 50 0.01];%75-35
                ub=[50 0.03  300 300 100];%75+35
                try
                   est_pix(i,j,:) = lsqcurvefit(fun_pix,[15 0.003 160 100 0.05],z,ydata,lb,ub,opts);
                catch
                    est_pix(i,j,:)=[0 0 0 0];
                end
            else
