function Optical_fitting_6692(co, cross, s_seg, z_seg, datapath)
...
%                 fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./60).^2))); %75; 3-parameter fitting using empirical rayleigh range
                ...
                lb = [0.0001 0.0001  110-20 50];% zf 110 for slice 21
                ub=[10 0.03 110+20 90];%
                try
                   [est_pix(i,j,:) rn] = lsqcurvefit(fun_pix,[0.01 0.006 110 70],z,ydata,lb,ub,opts);
