folder='/projectnb2/npbssmic/ns/Ann_7694/';
...
filename0=dir(strcat('ref-6-*.dat')); % count #tiles per slice
...
for islice=id
    cd(datapath);
    filename0=dir(strcat('ref-6-*.dat')); 
...
%         dim1=[100 1 1100 1 1100];
        ...
        % load reflectivity data
        ifilePath = [datapath,name1];
        ret = single(ReadDat_int16(ifilePath, [62 1 1000 1 1000]))./65535*180; 

        message=strcat('Tile No. ',string(iFile),' is read.', datestr(now,'DD:HH:MM'),'\n');
        fprintf(message);
        
%         start_pixel=40; % approx +40 px from straing point !!!!!!
...
%         t.close();
        ...
        Optical_fitting_7694(ref,ret,islice, iFile, folder);
    ...
    Mus_stitch_7694('mus',folder,folder,disp,mosaic,pxlsize./10,islice,pattern,sys);           % stitch mus
    Mub_stitch_7694('mub', folder,folder,disp,mosaic,pxlsize./10,islice,pattern,sys);
...
%     Bfg_stitch_7694('bfg', folder, folder,disp,mosaic,pxlsize./10,islice,pattern,sys)
%     MIP_stitch_7694('mip', folder,folder,disp,mosaic,pxlsize,islice,pattern,sys);
