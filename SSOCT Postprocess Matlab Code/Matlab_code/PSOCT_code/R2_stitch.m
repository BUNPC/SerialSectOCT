function R2_stitch(target, P2path, datapath,disp,mosaic,pxlsize,islice,pattern,sys,ds,stitch)
...
% %     
    ...
    filename0=strcat(datapath,'fitting/vol',num2str(islice),'/','R2.tif');
    filename0=dir(filename0);
    for iFile=1:numX*numY
        this_tile=iFile;
        R2 = double(imread(filename0(1).name, iFile));
...
%         filename0=dir(strcat(target,'-',num2str(islice),'-',num2str(in),'.mat'));
        ...
        if strcmp(sys,'PSOCT')
            ...
            Mosaic(row,column)=Mosaic(row,column)+R2.*Masque2(row,column); 
        elseif strcmp(sys,'Thorlabs')
            Mosaic(row,column)=Mosaic(row,column)+R2'.*Masque2(row,column);%#################################
        end
