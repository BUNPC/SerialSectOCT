function Optical_fitting_Hui_0505(ref, s_seg, z_seg, datapath)
...
%             xloc=tmp(ceil(i/50*res),ceil(j/50*res))+2;
            ...
            if m > 0.05
                ...
                fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./45).^2))) ; %75; 3-parameter fitting using empirical rayleigh range
    %             fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-est(3))./(est(4))).^2))); % 2-parameter fitting using tile-dependent zf and rayleigh range from above fitting
                ...
                lb = [0.001 0.0001 70 ];%75-35
                ub=[5 0.03  150 ];%75+35
                try
                   est_pix(i,j,:) = lsqcurvefit(fun_pix,[1 0.003 100 ],z,ydata,lb,ub,opts);
                catch
                    est_pix(i,j,:)=[0 0 0];
                end
            else
