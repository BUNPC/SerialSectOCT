function Optical_fitting_scattering_birefringence(co, cross, s_seg, z_seg, datapath)
...
for i = 1:round(size(I,2)/res1)
    for j = 1:round(size(I,3)/res1)
        area = k(:,(i-1)*res1+1:i*res1,(j-1)*res1+1:j*res1);
...
%                 fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./(120)).^2))); %75; 3-parameter fitting using empirical rayleigh range
            fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./0.122).^2))); % 4-parameter fitting using tile-dependent zf and rayleigh range from above fitting

            % upper and lower bound for fitting parameters
            lb = [0.001 0.1 0.05]; %[0.001 0.1  (105-3*xloc)/1000 (122-20)/1000];%75-35
