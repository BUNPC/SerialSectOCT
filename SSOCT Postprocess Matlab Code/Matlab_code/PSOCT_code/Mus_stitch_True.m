function Mus_stitch_True(datapath,disp,mosaic,pxlsize,islice,pattern,sys)
%%% ---------------------- %%%
...
%                     grid(1,i)=floor(i/numX)*yx;
    ...
    for j=starti:numTile
        filename0=dir(strcat('mus-',num2str(islice),'-',num2str(j),'.mat'));       % load AIP to determine whether it is agarose
%         if isexist(filename0.name)
%             filename0.name
            load(filename0.name);
            val(j)=std2(us);
            us=single(us);
            tiffname=strcat(datapath,'fitting/vol',num2str(islice),'/',num2str(j),'_us.tif');
            t = Tiff(tiffname,'w');
...
%         end
    ...
    filename = strcat(datapath,'fitting/vol',num2str(islice),'/');
    ...
    for i=1:length(index)
        ...
        filename0=dir(strcat('mus-',num2str(islice),'-',num2str(in),'.mat'));
        load(filename0.name);
