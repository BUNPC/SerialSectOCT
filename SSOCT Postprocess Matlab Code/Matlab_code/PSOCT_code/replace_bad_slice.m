...
if(strcmp(sys,'PSOCT'))
    folder_distort='/projectnb2/npbssmic/ns/201128_PSOCT_Ann_7694/';                     
...
cd(datapath);
filename0=dir(strcat('106-*AB.dat')); % count #tiles per slice
...
%                   
         ...
         if(strcmp(sys,'PSOCT'))
           start_point=102;
           name1=strcat(num2str(islice+2),'-',num2str(iFile),'-',num2str(size(cross,1)-start_point+1+72),'-',num2str(size(cross,2)),'-',num2str(size(cross,3)),'.dat'); % gen file name for reflectivity
           FILE_ref=strcat(corrected_path, 'replace/cross-', name1);
           FID=fopen(FILE_ref,'w');
           tmp=cross(start_point:end,:,:);
           tmp=[tmp;zeros(72,1000,1000)];
           fwrite(FID,tmp./2*65535,'int16');
           fclose(FID);
           
           name2=strcat(num2str(islice+2),'-',num2str(iFile),'-',num2str(size(co,1)-start_point+1+72),'-',num2str(size(co,2)),'-',num2str(size(co,3)),'.dat'); % gen file name for reflectivity
           FILE_ret=strcat(corrected_path, 'replace/co-', name2);
           FID=fopen(FILE_ret,'w');
           tmp=co(start_point:end,:,:);
           tmp=[tmp;zeros(72,1000,1000)];
           fwrite(FID,tmp./2*65535,'int16');
           fclose(FID);
         end
...
%          Optical_fitting(ref,islice, coord, datapath, 3);
         ...
         aip=squeeze(mean(ref(72:end,:,:),1));
         avgname=strcat(datapath,'aip/vol',num2str(slice_index+2),'/',coord,'.mat');
         mipname=strcat(datapath,'mip/vol',num2str(slice_index+2),'/',coord,'.mat');
         save(mipname,'mip');
         save(avgname,'aip');  
...
%          end
        ...
        % Saving AIP.tif
        aip=single(aip);
        tiffname=strcat(datapath,'aip/vol',num2str(slice_index+2),'/',coord,'_aip.tif');
        t = Tiff(tiffname,'w');
...
% mus, mub, surface and retardance.
    ...
    AIP_stitch(datapath,disp,mosaic,pxlsize,islice+2,pattern,sys);                     % stitch AIP
...
%     Mub_stitch('mub', datapath,disp,mosaic,pxlsize./10,islice,pattern,sys)           % stitch mub
    Surf_stitch('sur',datapath,disp,mosaic,pxlsize/10,islice+2,pattern,'PSOCT');              % stitch surface
