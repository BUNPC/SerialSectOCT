function Optical_fitting_7694(ref, ret, s_seg, z_seg, datapath)
...
%     ref=single(sqrt(co.^2+cross.^2));
    ...
    R2=zeros(round(size(vol,2)/res),round(size(vol,3)/res));
%     R=0;
...
%                 fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./60).^2))); %75; 3-parameter fitting using empirical rayleigh range
                ...
                lb = [0.0001 0.0001  80-20 50];%75-35
                ub=[10 0.03 80+20 90];%75+35
                try
                   [est_pix(i,j,:) rn] = lsqcurvefit(fun_pix,[0.01 0.006 80 70],z,ydata,lb,ub,opts);
...
% %             int = (int - mean(int(end-10:end-5)));
%             m=max(int);
%             xloc=ceil(sur(ceil(i/10*res),ceil(j/10*res))/4)+1;
%             if m > 0.01
%                 l=min(size(int,1)-5,xloc+fit_depth-1);
%                 ydata = double(int(xloc:l)');
...
%                 b=[0.01 1];%75+35
%                 try
%                    est_pix(i,j,:) = lsqcurvefit(fun_pix,[0.0002 0.05],z,ydata./4,lb,b,opts);
%                 catch
%                     est_pix(i,j,:)=[0 0];
%                 end
%             else
