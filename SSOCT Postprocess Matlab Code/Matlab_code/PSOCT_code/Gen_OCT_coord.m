...
%         end
    ...
    path1=strcat(datapath,'aip/vol',num2str(plane1),'/AIP.tif');
    fprintf(fid_Macro,'open("%s");\n',path1);
    fprintf(fid_Macro,'setMinAndMax("0.00","35000");\n');
    path2=strcat(datapath,'aip/vol',num2str(plane2),'/AIP.tif');
    fprintf(fid_Macro,'open("%s");\n',path2);
    fprintf(fid_Macro,'setMinAndMax("0.00","35000");\n');
    path3=strcat(datapath,'aip/vol',num2str(plane3),'/AIP.tif');
    fprintf(fid_Macro,'open("%s");\n',path3);
    fprintf(fid_Macro,'setMinAndMax("0.00","35000");\n');
    fprintf(fid_Macro,'run("Merge Channels...", "c1=AIP.tif c2=AIP-1.tif c3=AIP-2.tif create");\n');
    ...
    fprintf(fid_Macro,'close();\n');
    l=['run("Grid/Collection stitching", "type=[Positions from file] order=[Defined by TileConfiguration] directory=',filepath_rev,' layout_file=TileConfiguration.txt fusion_method=[Linear Blending] regression_threshold=0.01 max/avg_displacement_threshold=50 absolute_displacement_threshold=50 compute_overlap computation_parameters=[Save memory (but be slower)] image_output=[Write to disk] output_directory=',filepath_rev,'");\n'];
    fprintf(fid_Macro,l);
