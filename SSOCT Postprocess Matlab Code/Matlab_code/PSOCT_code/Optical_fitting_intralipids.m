function Optical_fitting_intralipids(co, cross, s_seg, z_seg, datapath)
...
% datapath: datapath for original data
    ...
    for i = 1:round(size(vol,2)/res)
        for j = 1:round(size(vol,3)/res)
            ...
            int = (int - mean(int(end-10:end-5)));
            m=max(int);
            xloc=sur(ceil(i/10*res),ceil(j/10*res))+26;
            if m > 0.05
                l=min(size(int,1)-5,xloc+fit_depth-1);
                ydata = double(int(xloc:l)');
                z = (26:(length(ydata)+25))*Z_step;
%                 fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-p(3))./60).^2))); %75; 3-parameter fitting using empirical rayleigh range
                ...
                lb = [0.0001 0.0001  80-30 30];%75-35
                ub=[10 0.03 300 200];%75+35
                try
                   [est_pix(i,j,:) rn] = lsqcurvefit(fun_pix,[0.6 0.006 200 120],z,ydata,lb,ub,opts);
...
%     save([datapath, '/fitting/vol', num2str(s_seg),'/',sa