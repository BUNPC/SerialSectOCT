function Optical_fitting_Hui_SupFrontal(ref, s_seg, z_seg, datapath)
...
%             xloc=sur(ceil(i/10*res),ceil(j/10*res))+2;
    ...
    %             fun_pix = @(p,zdata)sqrt(p(1).*exp(-2.*p(2).*(zdata)).*(1./(1+((zdata-est(3))./(est(4))).^2))); % 2-parameter fitting using tile-dependent zf and rayleigh range from above fitting
                ...
                lb = [0.1 0.0001 40];%75-35
                ub=[50 0.03  150];%75+35
                try
                   est_pix(i,j,:) = lsqcurvefit(fun_pix,[5 0.003 70],z,ydata,lb,ub,opts);
                catch
                    est_pix(i,j,:)=[0 0 0];
                end
            else
