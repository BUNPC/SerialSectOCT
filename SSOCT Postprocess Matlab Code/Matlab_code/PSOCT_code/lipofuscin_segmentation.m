...
% sampleID=["AD_10382_2P" "AD_20969_2P" "AD_21354_2P" "AD_21424_2P" "AD_20832_2P" ]; % 
% thresh=[2500,2500,2500,2500,2500];
sampleID=["CTE_6489_2P" "CTE_6912_2P" "CTE_7019_2P" "CTE_7126_2P" "CTE_8572_2P"];
thresh=[2500,2500,2500,3000,2000];
% sampleID=["NC_6047_2P" "NC_6839_2P" "NC_6974_2P" "NC_7597_2P" "NC_8095_2P"  "NC_8653_2P"  "NC_21499_2P"]; %NC_301189_2P
% thresh=[2500; 2500; 3000; 3000; 2500; 3000; 2500];
...
%         fprintf(fid_Macro,'saveAs("Tiff", "%s");\n',normed_path);
        ...
        image(mask>0)=0;
        SaveTiff(image,1,strcat(datapath,'channel2_',num2str(zz),'_seg.tif'));
    %% calculating lipofuscin distributions
        cd(datapath)
        fprintf(strcat("processing " ,datapath, num2str(zz),"\n"))
        % load segementation image
        image=single(imread(strcat(datapath,'channel2_',num2str(zz),'_seg.tif')));
        ...
        for x=1:round(kernel/2):X-kernel
            ...
            for y=1:round(kernel/2):Y-kernel
                ...
                for pp =1:cc.NumObjects
                    % find pixels inside each lipofuscin
                    [row,col]=ind2sub([kernel+1,kernel+1],cc.PixelIdxList{pp});
                    % remove lipofusion on the edge of ROI
...
%         caxis([0 15]);
    ...
    system(strcat("chmod -R 777 ",datapath));
end
