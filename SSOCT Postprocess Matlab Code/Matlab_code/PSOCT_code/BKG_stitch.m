function BKG_stitch(target, P2path, datapath,disp,mosaic,pxlsize,islice,pattern,sys,ds,stitch)
...
% %     
    ...
    filename0=strcat(datapath,'fitting/vol',num2str(islice),'/','BKG.tif');
    filename0=dir(filename0);
    for iFile=1:numX*numY
        this_tile=iFile;
        bkg = double(imread(filename0(1).name, iFile));
