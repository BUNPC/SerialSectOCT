...
%             xloc=sur(i,j)+5;
            if m > 0.0001
                if exist('us','var')==0
                    us=imread(strcat(datapath,'fitting_4x/vol',num2str(s_seg),'/','MUS.tif'),z_seg);
                end
                bfg_depth1=round(bfg_depth-(us(ceil(i/4*res),ceil(j/4*res))-3)*4); %2 for NC6839,NC6974; 4 for NC6074,NC6839,NC21499
