function[]=ref_vol_stitch_from_AB(id,datapath, depth, thickness)
Xsize=1000;
Ysize=1000;
Xoverlap=0.15;
Yoverlap=0.15;
...
for nslice=id
    ...
    for i=1:length(index)
        ...
        slice = slice(depth:end,:,:); %25 for PTSD1_1 %16 for PTSD1_2 %41 for PTSD1_3
        vol=imresize3(slice,resize_factor);
        tic
%         vol = depth_corr(vol,0.0035);
        toc
        vol=vol(1:thickness*resize_factor,:,:);
...
%     tiffname=strcat(datapath,'volume/Ref',num2str(nslice),'.tif');
%     for i=1:size(Ref,3)
%         t = Tiff(tiffname,'a');
...
%     end
    ...
    saveastiff(Ref, strcat(datapath,'volume/ref_raw',num2str(id),'.btf'), options);
