...
% Unpublished work as of April 26, 2020
%%
addpath(genpath('.'));
clc;clear;close all;
%%
test_data = load('Test_data\Graph_refinemnt_test_data.mat');
%%
%%
...
%% Use vessel mask got from Graph
[filename,pathname] = uigetfile({'*.mat;*.tiff;*.tif'},'Please select the vessel skeleton Data');
h = waitbar(0,'Please wait... loading the data');
[~,~,ext] = fileparts(filename);
...
opt.select_dim_short_link.min_int_max = 1;%22000; %(I should set based on cap intensity)
...
% Training data
TD = struct;
TD.info.filepath = fullfile('Test_data', 'Test_training_data.mat');
opt.record.TD = TD;
%% Graph refinement 
...
% illustration
%%
classifier_folder = fullfile('Test_data', 'Pretrained_classifier');
