function [vessel_graph, record] = fun_graph_refine_by_classifier(vessel_graph, vessel_image, vessel_mask_dt, opt)
...
if ~opt.annotation_on_Q
    ...
    if isfield(opt.classifier, 'link_ep1_af')
        c_link_ep1_af = opt.classifier.link_ep1_af;
        if isfield(opt, 'min_artefact_link_ep1_length')
            min_artefact_link_ep1_length = opt.min_artefact_link_ep1_length;
        else
            min_artefact_link_ep1_length = 15;
        end
    else
        c_link_ep1_af = [];
    end
...
%% Remvoe self-loop and small bilink loop 
[vessel_graph, pruning_info_str_1] = fun_graph_delete_hairs_and_short_loops(vessel_graph, vessel_image, opt);
record.num_self_loop = record.num_self_loop + pruning_info_str_1.num_self_loop;
record.num_bilink_loop = record.num_bilink_loop + pruning_info_str_1.num_bilink_loop;
record.num_short_link_ep1 = record.num_short_link_ep1 + pruning_info_str_1.num_short_link_ep1;
...
if int_link_0.ep1.num_cc > 0
    ...
    if ~annotation_onQ
        ...
        if ~isempty(c_link_ep1_af)
            % Remove link with one endpoints generated by stitching
            % artefact (abrupt illumination changes in adjacent z sections)
            % Also have the false positive rate... 
            tmp_feature = table2array(tmp_ep1_features(:, c_link_ep1_af.used_feature_name));
            tmp_ep1_link_artefact_removed_Q = c_link_ep1_af.classifier.predict(tmp_feature);
            % Only delete the links that are longer than a minimum length: 
            tmp_ep1_link_artefact_removed_Q = tmp_ep1_link_artefact_removed_Q & tmp_ep1_features.length >= min_artefact_link_ep1_length;
            tmp_ep1_link_removed_Q = tmp_ep1_link_removed_Q | tmp_ep1_link_artefact_removed_Q;
        end        
    else
        % For visualization
        ...
        clearvars vessel_skl_rc vessel_mask
...
[vessel_graph, tmp_clear_str] = fun_graph_delete_hairs_and_short_loops(vessel_graph, vessel_image, opt);
record.num_self_loop = record.num_self_loop + tmp_clear_str.num_self_loop;
record.num_bilink_loop = record.num_bilink_loop + tmp_clear_str.num_bilink_loop;
record.num_short_link_ep1 = record.num_short_link_ep1 + tmp_clear_str.num_short_link_ep1;
...
duplicated_link_label = fun_graph_get_duplicated_ep2_link_label(int_link_1, int_link_2);
vessel_graph = fun_graph_pruning_by_link_label(vessel_graph, duplicated_link_label);
[vessel_graph, tmp_stat]= fun_graph_delete_hairs_and_short_loops(vessel_graph, vessel_image, opt);
% Record for next iteration of refinement
record.num_self_loop = record.num_self_loop + tmp_stat.num_self_loop;
record.num_bilink_loop = record.num_bilink_loop + tmp_stat.num_bilink_loop;
record.num_short_link_ep1 = record.num_short_link_ep1 + tmp_stat.num_short_link_ep1;
int_link_2 = fun_graph_get_free_link(vessel_graph, internal_offset);
