    ...
    for (boardIndex = 0; (boardIndex < boardCount) && success; boardIndex++)
    {
        ...
        if (success)
        {
            retCode = AlazarBeforeAsyncRead(
                ...
                ADMA_EXTERNAL_STARTCAPTURE | ADMA_TRIGGERED_STREAMING | ADMA_FIFO_ONLY_STREAMING);
            if (retCode != ApiSuccess)
            {
        ...
        while (buffersCompletedPerBoard < buffersPerAcquisition)
        {
            ...
            for (boardIndex = 0; (boardIndex < boardCount) && success; boardIndex++)
            {
                ...
                if (success)
                {
                    ...
                    //
                    // A 14-bit sample code is stored in the most significant
                    // bits of
