...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9870 to make a average
// NPT acquisition.
//
            ...
            if (success)
            {
                ...
                if (success)
                {
                    ...
                    for (U32 sample = 0; sample < samplesPerRecord * channelCount; sample++)
                        pAveRecord[sample] = (U8)(*pAccumulatorSample++ / recordsPerBuffer);

                    // Save the average record to file
                    if (saveAverage)
                    {
                        size_t samplesWritten = fwrite(pAveRecord, sizeof(U8), channelCount * samplesPerRecord, fpAverage);
