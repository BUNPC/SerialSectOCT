...
#endif
    U8 *pRecord;    // Address of first record in DMA buffer for this thread sum
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    U8 *pAveRecord = (U8*) malloc(samplesPerRecord * channelCount * sizeof(U8));
    if (pAveRecord == NULL)
    {
        printf("Error: Alloc %u bytes failed\n", samplesPerRecord * channelCount * sizeof(U8));
        success = FALSE;
    }
	else
	{
            ...
            if (success)
            {
                ...
                if (success)
                {
                    ...
                    for (U32 sample = 0; sample < samplesPerRecord; sample++)
                    {
                        ...
                        *(pAveRecord + sample) = (U8)(sum / recordsPerBuffer);
                    }
...
#endif
{
    ...
    for (;;)
    {
        ...
        U8 *pRecord = pThreadParams->pRecord;
