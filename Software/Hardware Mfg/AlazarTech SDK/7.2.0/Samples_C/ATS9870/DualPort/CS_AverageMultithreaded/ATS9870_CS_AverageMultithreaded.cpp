...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9870 to make a multithreaded average
// CS acquisition.
//
...
#endif
    U8 *pSamples;    // Address in DMA buffer of the first sample for this thread to average
    U8 *pAveSamples;  // Address in averaged data buffer for this thread to store the first averaged
                      // value
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    U8 *pAveBuffer = (U8*) malloc(bytesPerAverageBuffer);
    if (pAveBuffer == NULL)
    {
...
#endif
{
    ...
    for (;;)
    {
        ...
        U8 *pSample = pThreadParams->pSamples;
        U8 *pAveSamples = pThreadParams->pAveSamples;
        ...
        if (pThreadParams->channelsPerBuffer > 1)
        {
            ...
            for (U32 point = 0; point < pointsToAverage; point++)
            {
                ...
                *(pAveSamples + point) = (U8)(sumA / samplesPerPoint);
                *(pAveSamples + point + pThreadParams->pointsPerBufferPerChannel) =
                    (U8)(sumB / samplesPerPoint);
            }
        }
        else
        {
            ...
            for (U32 point = 0; point < pointsToAverage; point++)
            {
                ...
                *(pAveSamples + point) = (U8)(sum / samplesPerPoint);
            }
        }
