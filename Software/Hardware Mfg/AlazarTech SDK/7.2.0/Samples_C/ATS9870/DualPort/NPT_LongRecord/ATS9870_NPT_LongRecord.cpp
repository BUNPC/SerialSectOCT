...
// AcqToDisk.cpp :
//
// This program demonstrates how to configure a ATS9870 to make a long record
...
U8 *RecordArray[ALLOCATED_RECORD_COUNT] = { NULL };
...
BOOL AcquireData(HANDLE boardHandle)
{
    ...
    for (recordIndex = 0; (recordIndex < ALLOCATED_RECORD_COUNT) && success; recordIndex++)
    {
        ...
        RecordArray[recordIndex] = (U8 *)AlazarAllocBufferU8Ex(boardHandle, bytesPerRecord);
    ...
    while (totalBuffersPosted < POSTED_BUFFER_COUNT) {
        ...
        U8 *pBuffer = RecordArray[recordIndex % ALLOCATED_RECORD_COUNT];
        pBuffer = pBuffer + bytesPerBuffer * bufferIndexInRecord;
    ...
    if (success)
    {
        ...
        while (recordsCompleted < recordsPerAcquisition)
        {
            ...
            for (int transfer = 0; transfer < transfersPerRecord; transfer++) {
                U8 *pBuffer = RecordArray[recordsCompleted % ALLOCATED_RECORD_COUNT];
                pBuffer = pBuffer + transfer * bytesPerBuffer;
            ...
            if (success)
            {
                ...
                U8 *pRecord = RecordArray[recordsCompleted % ALLOCATED_RECORD_COUNT];
    ...
    for (recordIndex = 0; recordIndex < ALLOCATED_RECORD_COUNT; recordIndex++)
    {
        if (RecordArray[recordIndex] != NULL)
        {
            AlazarFreeBufferU8(boardHandle, RecordArray[recordIndex]);
        }
    }
