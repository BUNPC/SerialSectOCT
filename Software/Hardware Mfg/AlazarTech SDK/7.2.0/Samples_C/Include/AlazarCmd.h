...
enum ALAZAR_PARAMETERS_UL {
    ...
    ///
    /// The value returned is zero if there is no problem. If it is not zero,
    /// please send the value returned to AlazarTech's technical support.
...
#define DATA_WIDTH 0x10000009UL
 ...
 */
enum ALAZAR_LED { LED_OFF = 0x00000000UL, LED_ON = 0x00000001UL };
 ...
 * @brief Power states
 */
enum ALAZAR_POWER_STATES { POWER_OFF = 0x00000000UL, POWER_ON = 0x00000001UL };
...
enum ALAZAR_AUX_IO_MODES {
    ...
    /// Uses the edge of a pulse to the AUX I/O connector as an AutoDMA *trigger
    ...
    /// ALAZAR_TRIGGER_SLOPES, which defines on which edge of the input signal a
    /// trigger enable event is generated.
    AUX_IN_TRIGGER_ENABLE = 1U,

    /// Output the sample clock divided by the value passed to the \c parameter
    /// argument of AlazarConfigureAuxIO(). Please note that the divided must be
    /// greater than 2.
    AUX_OUT_PACER = 2U,

    /// Use the AUX I/O connector as a general purpose digital output. The \c
    ...
    /// AlazarGetParameter() with \ref GET_AUX_INPUT_LEVEL to read the digital
    /// input level.
 ...
 */
enum ALAZAR_SAMPLE_SKIPPING_MODES { SSM_DISABLE = 0, SSM_ENABLE = 1 };
